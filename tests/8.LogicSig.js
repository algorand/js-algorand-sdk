...
describe('LogicSig functionality', () => {
  describe('Basic logic sig', () => {
    it('should work on valid program', () => {
      ...
      const programHash = '6Z3C3LDVWGMX23BMSYMANACQOSINPFIRF77H7N3AWJZYV6OH6GWTJKVMXY';
  ...
  describe('Signatures', () => {
    it('should sign a basic transaction', () => {
      ...
      const genesisID = '';
      const rekeyTo = 'GAQVB24XEPYOPBQNJQAE4K3OLNYTRYD65ZKR3OEW5TDOOGL7MDKABXHHTM';
...
describe('Logic validation', () => {
  ...
  describe('Const blocks', () => {
    ...
    it('should parse bytes const block correctly', () => {
      const data = Uint8Array.from([38, 2, 13, 49, 50, 51, 52, 53, 54, 55, 56, 57, 48, 49, 50, 51, 2, 1, 2]);
...
describe('Template logic validation', () => {
  describe('Split', () => {
    it('should match the goldens', () => {
      // Inputs
      const owner = 'WO3QIJ6T4DZHBX5PWJH26JLHFSRT7W7M2DJOULPXDTUS6TUX7ZRIO4KDFY';
      const receivers = ['W6UUUSEAOGLBHT7VFT4H2SDATKKSG6ZBUIJXTZMSLW36YS44FRP5NVAU7U', 'XCIBIN7RT4ZXGBMVAMU3QS6L5EKB7XGROC5EPCNHHYXUIBAA5Q6C5Y7NEU'];
      const rat1 = 100;
      const rat2 = 30;
      const expiryRound = 123456;
      const minPay = 10000;
      const maxFee = 5000000;
      const split = new algosdk.LogicTemplates.Split(owner, receivers[0], receivers[1], rat1, rat2, expiryRound, minPay, maxFee);
      // Outputs
      const goldenProgram = '';
      ...
      const goldenAddress = 'KPYGWKTV7CKMPMTLQRNGMEQRSYTYDHUOFNV4UDSBDLC44CLIJPQWRTCPBU';
  ...
  describe('HTLC', () => {
    it('sha256 should match the goldens', () => {
      // Inputs
      const owner = '726KBOYUJJNE5J5UHCSGQGWIBZWKCBN4WYD7YVSTEXEVNFPWUIJ7TAEOPM';
      ...
      const maxFee = 1000;
      const htlc = new algosdk.LogicTemplates.HTLC(owner, receiver, hashFn, hashImg, expiryRound, maxFee);
      // Outputs
      const goldenProgram = 'ASAE6AcBAMDPJCYDIOaalh5vLV96yGYHkmVSvpgjXtMzY8qIkYu5yTipFbb5IBB2YRNPIfx8AiI9UKues2ALw//DcSQjoeR7sfmp2/VfIP68oLsUSlpOp7Q4pGgayA5soQW8tgf8VlMlyVaV9qITMQEiDjEQIxIQMQcyAxIQMQgkEhAxCSgSLQEpEhAxCSoSMQIlDRAREA==';
      const goldenBytes = Buffer.from(goldenProgram, 'base64');
      const actualBytes = htlc.getProgram();
      assert.deepStrictEqual(goldenBytes, actualBytes);
      const goldenAddress = 'FBZIR3RWVT2BTGVOG25H3VAOLVD54RTCRNRLQCCJJO6SVSCT5IVDYKNCSU';
      assert.deepStrictEqual(goldenAddress, htlc.getAddress());
      const goldenLtxn = '';
      const o = {
        from: goldenAddress,
        to: receiver,
        fee: 0,
      ...
      const actualTxn = algosdk.LogicTemplates.signTransactionWithHTLCUnlock(htlc.getProgram(), o, preImageAsBase64);
    ...
    it('keccak256 should match the goldens', () => {
      ...
      const htlc = new algosdk.LogicTemplates.HTLC(owner, receiver, hashFn, hashImg, expiryRound, maxFee);
      // Outputs
      const goldenProgram = 'ASAE6AcBAMDPJCYDIOaalh5vLV96yGYHkmVSvpgjXtMzY8qIkYu5yTipFbb5IA+3eDK7wazksjUplLNJM7nLifv/Q2y5AOjjjqHMsKAHIP68oLsUSlpOp7Q4pGgayA5soQW8tgf8VlMlyVaV9qITMQEiDjEQIxIQMQcyAxIQMQgkEhAxCSgSLQIpEhAxCSoSMQIlDRAREA==';
      const goldenBytes = Buffer.from(goldenProgram, 'base64');
      const actualBytes = htlc.getProgram();
      assert.deepStrictEqual(goldenBytes, actualBytes);
      const goldenAddress = '3MJ6JY3P6AU4R6I2RASYSAOPNI3QMWPZ7HYXJRNRGBIAXCHAY7QZRBH5PQ';
      assert.deepStrictEqual(goldenAddress, htlc.getAddress());
  ...
  describe('Limit Order', () => {
    it('should match the goldens', () => {
      ...
      const maxFee = 5000000;
      const limitOrder = new algosdk.LogicTemplates.LimitOrder(owner, assetid, ratn, ratd, expiryRound, minTrade, maxFee);
      // Outputs
      const goldenProgram = 'ASAKAAHAlrECApBOBLlgZB7AxAcmASD+vKC7FEpaTqe0OKRoGsgObKEFvLYH/FZTJclWlfaiEzEWIhIxECMSEDEBJA4QMgQjEkAAVTIEJRIxCCEEDRAxCTIDEhAzARAhBRIQMwERIQYSEDMBFCgSEDMBEzIDEhAzARIhBx01AjUBMQghCB01BDUDNAE0Aw1AACQ0ATQDEjQCNAQPEEAAFgAxCSgSMQIhCQ0QMQcyAxIQMQgiEhAQ';
      const goldenBytes = Buffer.from(goldenProgram, 'base64');
      const actualBytes = limitOrder.getProgram();
      assert.deepStrictEqual(goldenBytes, actualBytes);
      const goldenAddress = 'LXQWT2XLIVNFS54VTLR63UY5K6AMIEWI7YTVE6LB4RWZDBZKH22ZO3S36I';
      assert.deepStrictEqual(goldenAddress, limitOrder.getAddress());
      const secretKey = Buffer.from('DTKVj7KMON3GSWBwMX9McQHtaDDi8SDEBi0bt4rOxlHNRahLa0zVG+25BDIaHB1dSoIHIsUQ8FFcdnCdKoG+Bg==', 'base64');
      const actualBlob = algosdk.LogicTemplates.getSwapAssetsTransaction(actualBytes, 3000, 10000, secretKey, 10, 1234, 2234, 'f4OxZX/x/FO5LcGBSKHWXfwtSx+j1ncoSt3SABJtkGk=');
      ...
      const expectedTxn2 = Buffer.from('', 'base64');
  ...
  describe('Periodic payment', () => {
    it('should match the goldens', () => {
      // Inputs
      const receiver = 'SKXZDBHECM6AS73GVPGJHMIRDMJKEAN5TUGMUPSKJCQ44E6M6TC2H2UJ3I';
      ...
      const maxFee = 1000;
      const periodicPayment = new algosdk.LogicTemplates.PeriodicPayment(receiver, amount, withdrawalWindow, period, expiryRound, maxFee, leaseb64);
      // Outputs
      ...
      assert.deepStrictEqual(goldenBytes, actualBytes);
      const goldenAddress = 'JMS3K4LSHPULANJIVQBTEDP5PZK6HHMDQS4OKHIMHUZZ6OILYO3FVQW7IY';
  ...
  describe('Dynamic Fee', () => {
    it('should match the goldens', () => {
      // Inputs
      const receiver = '726KBOYUJJNE5J5UHCSGQGWIBZWKCBN4WYD7YVSTEXEVNFPWUIJ7TAEOPM';
      const amount = 5000;
      const firstValid = 12345;
      const lastValid = 12346;
      const closeRemainder = '42NJMHTPFVPXVSDGA6JGKUV6TARV5UZTMPFIREMLXHETRKIVW34QFSDFRE';
      ...
      const dynamicFee = new algosdk.LogicTemplates.DynamicFee(receiver, amount, firstValid, lastValid, closeRemainder, leaseBytes);
      // Outputs
      const goldenProgram = 'ASAFAgGIJ7lgumAmAyD+vKC7FEpaTqe0OKRoGsgObKEFvLYH/FZTJclWlfaiEyDmmpYeby1feshmB5JlUr6YI17TM2PKiJGLuck4qRW2+SB/g7Flf/H8U7ktwYFIodZd/C1LH6PWdyhK3dIAEm2QaTIEIhIzABAjEhAzAAcxABIQMwAIMQESEDEWIxIQMRAjEhAxBygSEDEJKRIQMQgkEhAxAiUSEDEEIQQSEDEGKhIQ';
      const goldenBytes = Buffer.from(goldenProgram, 'base64');
      const actualBytes = dynamicFee.getProgram();
      assert.deepStrictEqual(goldenBytes, actualBytes);
      const goldenAddress = 'GCI4WWDIWUFATVPOQ372OZYG52EULPUZKI7Y34MXK3ZJKIBZXHD2H5C5TI';
      assert.deepStrictEqual(goldenAddress, dynamicFee.getAddress());
      const privateKeyOneB64 = 'cv8E0Ln24FSkwDgGeuXKStOTGcze5u8yldpXxgrBxumFPYdMJymqcGoxdDeyuM8t6Kxixfq0PJCyJP71uhYT7w==';
      const privateKeyOne = Buffer.from(privateKeyOneB64, 'base64');
      const goldenGenesisHash = 'f4OxZX/x/FO5LcGBSKHWXfwtSx+j1ncoSt3SABJtkGk=';
      const txnAndLsig = algosdk.LogicTemplates.signDynamicFee(actualBytes, privateKeyOne, goldenGenesisHash);
      const txnDict = txnAndLsig.txn;
      const txnObj = new algosdk.Transaction(txnDict);
      const txnBytes = txnObj.toByte();
      const goldenTxn = '';
