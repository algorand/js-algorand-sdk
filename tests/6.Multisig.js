...
describe('Multisig Functionality', () => {
  describe('should generate correct partial signature', () => {
    it('first partial sig should match golden main repo result', () => {
      ...
      const mnem2 = 'since during average anxiety protect cherry club long lawsuit loan expand embark forum theory winter park twenty ball kangaroo cram burst board host ability left';
    ...
    it('second partial sig should match golden main repo result', () => {
      ...
      const finMsigBlob = multisig.mergeMultisigTransactions([msigBlob, oneSigTxn]);
  ...
  describe('should sign keyreg transaction types', () => {
    it('first partial sig should match golden main repo result', () => {
      ...
      const msigTxn = multisig.MultisigTransaction.from_obj_for_encoding(decRawTx);
      const mnem1 = 'auction inquiry lava second expand liberty glass involve ginger illness length room item discover ahead table doctor term tackle cement bonus profit right above catch';
      const { sk } = algosdk.mnemonicToSecretKey(mnem1);
      const msigBlob = msigTxn.partialSignTxn(params, sk);
    ...
    it('second partial sig with 3rd pk should match golden main repo result', () => {
      ...
      const finMsigBlob = multisig.mergeMultisigTransactions([msigBlob, new Uint8Array(rawOneSigTxBlob)]);
