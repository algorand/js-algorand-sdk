...
module.exports = function getSteps(options) {
  ...
  When('I update the asset index', async function () {
    const accountResponse = await this.acl.accountInformation(this.assetTestFixture.creator);
  ...
  Then('the creator should have {int} assets remaining', async function (expectedTotal) {
    const accountInformation = await this.acl.accountInformation(this.assetTestFixture.creator);
  ...
  When('I send the split transactions', async function () {
    ...
    this.lv = this.fv + 1000;
    this.lastRound = this.params.lastRound;
    const amount = this.contractTestFixture.splitMin * (this.contractTestFixture.splitRat1 + this.contractTestFixture.splitRat2);
    const txnBytes = algosdk.LogicTemplates.getSplitFundsTransaction(contractCode, amount, this.fv, this.lv, this.fee, this.params.genesishashb64);
  ...
  When('I claim the algos', async function () {
    ...
    const txnBytes = algosdk.LogicTemplates.signTransactionWithHTLCUnlock(this.contractTestFixture.htlc.getProgram(), payTxn, this.contractTestFixture.htlcPreimage);
  ...
  When('I claim the periodic payment', async function () {
    ...
    this.lastRound = this.params.lastRound;
    this.gh = this.params.genesishashb64;
    const txnBytes = algosdk.LogicTemplates.getPeriodicPaymentWithdrawalTransaction(this.contractTestFixture.periodicPay.getProgram(), this.fee, this.fv, this.gh);
    this.txid = await this.acl.sendRawTransaction(txnBytes.blob);
    this.txid = this.txid.txId;
    this.pk = this.contractTestFixture.activeAddress;
  });
  ...
  When('I swap assets for algos', async function () {
    const response = await this.kcl.exportKey(this.handle, this.wallet_pswd, this.accounts[1]);
    const secretKey = response.private_key;
    const microAlgoAmount = this.contractTestFixture.limitOrderMin + 1; // just over the minimum
    const assetAmount = Math.floor(microAlgoAmount * this.contractTestFixture.limitOrderN / this.contractTestFixture.limitOrderD) + 1;
    this.params = await this.acl.getTransactionParams();
    this.fee = this.params.fee;
    this.fv = this.params.lastRound;
    this.lv = this.fv + 1000;
    this.lastRound = this.params.lastRound;
    this.gh = this.params.genesishashb64;
    const txnBytes = algosdk.LogicTemplates.getSwapAssetsTransaction(this.contractTestFixture.limitOrder.getProgram(), assetAmount, microAlgoAmount, secretKey, this.fee, this.fv, this.lv, this.gh);
    this.txid = await this.acl.sendRawTransaction(txnBytes);
    this.txid = this.txid.txId;
    this.pk = this.contractTestFixture.activeAddress;
  });
  ...
  Given('I send the dynamic fee transactions', async function () {
    ...
    const firstResponse = await this.kcl.exportKey(this.handle, this.wallet_pswd, this.accounts[0]);
    const secretKeyOne = firstResponse.private_key;
    const txnAndLsig = algosdk.LogicTemplates.signDynamicFee(this.contractTestFixture.dynamicFee.getProgram(), secretKeyOne, this.gh);
    const secondResponse = await this.kcl.exportKey(this.handle, this.wallet_pswd, this.accounts[1]);
    const secretKeyTwo = secondResponse.private_key;
    const txnBytes = algosdk.LogicTemplates.getDynamicFeeTransactions(txnAndLsig.txn, txnAndLsig.lsig, secretKeyTwo, this.fee);
    this.txid = await this.acl.sendRawTransaction(txnBytes);
    this.txid = this.txid.txId;
    this.pk = this.accounts[0];
  });
  ...
  Given('mock http responses in {string} loaded from {string}', function (expectedBody) {
  ...
  Then('expect the path used to be {string}', (algodSeenRequests, indexerSeenRequests, expectedRequestPath) => {
  ...
  Then('we expect the path used to be {string}', (algodSeenRequests, indexerSeenRequests, expectedRequestPath) => {
    let actualRequestPath;
    if (algodSeenRequests.length != 0) {
  ...
  Then('the parsed Pending Transaction Information response should have sender {string}', (sender) => {
    const actualSender = algosdk.encodeAddress(anyPendingTransactionInfoResponse.txn.txn.snd);
  ...
  Then('the parsed Pending Transactions Information response should contain an array of len {int} and element number {int} should have sender {string}', (len, idx, sender) => {
  ...
  Then('the parsed Send Raw Transaction response should have txid {string}', (txid) => {
  ...
  Then('the parsed Pending Transactions By Address response should contain an array of len {int} and element number {int} should have sender {string}', (len, idx, sender) => {
  ...
  Then('the parsed Node Status response should have a last round of {int}', (lastRound) => {
  ...
  Then('the parsed Ledger Supply response should have totalMoney {int} onlineMoney {int} on round {int}', (totalMoney, onlineMoney, round) => {
  ...
  Then('the parsed Status After Block response should have a last round of {int}', (lastRound) => {
    assert.strictEqual(lastRound, anyNodeStatusResponse['last-round']);
  });
  ...
  Then('the parsed Account Information response should have address {string}', (address) => {
  ...
  Then('the parsed Get Block response should have rewards pool {string}', (rewardsPoolAddress) => {
    const rewardsPoolB64String = Buffer.from(anyBlockResponse.block.rwd).toString('base64');
  ...
  Then('the parsed Suggested Transaction Parameters response should have first round valid of {int}', (firstRound) => {
  ...
  When('we make a Lookup Asset Balances call against asset index {int} with limit {int} nextToken {string} round {int} currencyGreaterThan {int} currencyLessThan {int}', async function (index, limit, nextToken, round, currencyGreater, currencyLesser) {
    await this.indexerClient.lookupAssetBalances(index).limit(limit).round(round).currencyGreaterThan(currencyGreater)
  ...
  When('we make a Lookup Asset Transactions call against asset index {int} with NotePrefix {string} TxType {string} SigType {string} txid {string} round {int} minRound {int} maxRound {int} limit {int} beforeTime {int} afterTime {int} currencyGreaterThan {int} currencyLessThan {int} address {string} addressRole {string} ExcluseCloseTo {string}', async function (assetIndex, notePrefix, txType, sigType, txid, round, minRound, maxRound, limit, beforeTime, afterTime, currencyGreater, currencyLesser, address, addressRole, excludeCloseToAsString) {
    ...
    await this.indexerClient.lookupAssetTransactions(assetIndex).notePrefix(notePrefix).txType(txType).sigType(sigType)
      .txid(txid)
      .round(round)
      .minRound(minRound)
      .maxRound(maxRound)
  ...
  When('we make a Lookup Account Transactions call against account {string} with NotePrefix {string} TxType {string} SigType {string} txid {string} round {int} minRound {int} maxRound {int} limit {int} beforeTime {string} afterTime {string} currencyGreaterThan {int} currencyLessThan {int} assetIndex {int}', async function (account, notePrefix, txType, sigType, txid, round, minRound, maxRound, limit, beforeTime, afterTime, currencyGreater, currencyLesser, assetIndex) {
    await this.indexerClient.lookupAccountTransactions(account).notePrefix(notePrefix).txType(txType).sigType(sigType)
      .txid(txid)
      .round(round)
      .minRound(minRound)
      .maxRound(maxRound)
  ...
  When('we make a Search Accounts call with assetID {int} limit {int} currencyGreaterThan {int} currencyLessThan {int} and nextToken {string}', async function (assetIndex, limit, currencyGreater, currencyLesser, nextToken) {
    await this.indexerClient.searchAccounts().assetID(assetIndex).limit(limit).currencyGreaterThan(currencyGreater)
      .currencyLessThan(currencyLesser)
      .nextToken(nextToken)
      .do();
  });
  ...
  When('we make a Search For Transactions call with account {string} NotePrefix {string} TxType {string} SigType {string} txid {string} round {int} minRound {int} maxRound {int} limit {int} beforeTime {int} afterTime {int} currencyGreaterThan {int} currencyLessThan {int} assetIndex {int} addressRole {string} ExcluseCloseTo {string}', async function (account, notePrefix, txType, sigType, txid, round, minRound, maxRound, limit, beforeTime, afterTime, currencyGreater, currencyLesser, assetIndex, addressRole, excludeCloseToAsString) {
    ...
    await this.indexerClient.searchForTransactions().address(account).notePrefix(notePrefix).txType(txType)
      .sigType(sigType)
      .txid(txid)
      .round(round)
      .minRound(minRound)
  ...
  When('we make a SearchForAssets call with limit {int} creator {string} name {string} unit {string} index {int} and nextToken {string}', async function (limit, creator, name, unit, index, nextToken) {
    await this.indexerClient.searchForAssets().limit(limit).creator(creator).name(name)
      .unit(unit)
      .index(index)
      .nextToken(nextToken)
      .do();
  ...
  Then('the parsed LookupAssetBalances response should be valid on round {int}, and contain an array of len {int} and element number {int} should have address {string} amount {int} and frozen state {string}', (round, length, idx, address, amount, frozenStateAsString) => {
  ...
  Then('the parsed LookupAssetTransactions response should be valid on round {int}, and contain an array of len {int} and element number {int} should have sender {string}', (round, length, idx, sender) => {
  ...
  Then('the parsed LookupAccountTransactions response should be valid on round {int}, and contain an array of len {int} and element number {int} should have sender {string}', (round, length, idx, sender) => {
  ...
  Then('the parsed LookupAccountByID response should have address {string}', (address) => {
  ...
  Then('the parsed SearchAccounts response should be valid on round {int} and the array should be of len {int} and the element at index {int} should have address {string}', (round, length, idx, address) => {
  ...
  Then('the parsed SearchAccounts response should be valid on round {int} and the array should be of len {int} and the element at index {int} should have authorizing address {string}', (round, length, idx, authAddress) => {
    assert.strictEqual(round, anySearchAccountsResponse['current-round']);
    assert.strictEqual(length, anySearchAccountsResponse.accounts.length);
    if (length == 0) {
      return;
  ...
  Then('the parsed SearchForTransactions response should be valid on round {int} and the array should be of len {int} and the element at index {int} should have sender {string}', (round, length, idx, sender) => {
  ...
  Then('the parsed SearchForTransactions response should be valid on round {int} and the array should be of len {int} and the element at index {int} should have rekey-to {string}', (round, length, idx, rekeyTo) => {
    assert.strictEqual(round, anySearchForTransactionsResponse['current-round']);
    assert.strictEqual(length, anySearchForTransactionsResponse.transactions.length);
    if (length == 0) {
      return;
  ...
  Then('the parsed SearchForAssets response should be valid on round {int} and the array should be of len {int} and the element at index {int} should have asset index {int}', (round, length, idx, assetIndex) => {
  ...
  Given('indexer client {int} at {string} port {int} with token {string}', (clientNum, indexerHost, indexerPort, indexerToken) => {
  ...
  Then('The block was confirmed at {int}, contains {int} transactions, has the previous block hash {string}', (timestamp, numTransactions, prevHash) => {
  ...
  When('I use {int} to lookup account {string} at round {int}', async (clientNum, account, round) => {
    const ic = indexerIntegrationClients[clientNum];
    integrationLookupAccountResponse = await ic.lookupAccountByID(account).round(round).do();
  });

  Then('The account has {int} assets, the first is asset {int} has a frozen status of {string} and amount {int}.', (numAssets, firstAssetIndex, firstAssetFrozenStatus, firstAssetAmount) => {
    ...
    const scrutinizedAsset = integrationLookupAccountResponse.account.assets[0];
  ...
  Then('The account created {int} assets, the first is asset {int} is named {string} with a total amount of {int} {string}', (numCreatedAssets, firstCreatedAssetIndex, assetName, assetIssuance, assetUnit) => {
    assert.strictEqual(numCreatedAssets, integrationLookupAccountResponse.account['created-assets'].length);
    const scrutinizedAsset = integrationLookupAccountResponse.account['created-assets'][0];
  ...
  Then('The account has {int} μalgos and {int} assets, {int} has {int}', (microAlgos, numAssets, assetIndexToScrutinize, assetAmount) => {
    ...
    for (idx = 0; idx < integrationLookupAccountResponse.account.assets.length; idx++) {
      const scrutinizedAsset = integrationLookupAccountResponse.account.assets[idx];
  ...
  Then('The asset found has: {string}, {string}, {string}, {int}, {string}, {int}, {string}', (name, units, creator, decimals, defaultFrozen, totalIssuance, clawback) => {
  ...
  When('I use {int} to lookup asset balances for {int} with {int}, {int}, {int} and token {string}', async (clientNum, assetIndex, currencyGreater, currencyLesser, limit, nextToken) => {
    const ic = indexerIntegrationClients[clientNum];
    integrationLookupAssetBalancesResponse = await ic.lookupAssetBalances(assetIndex).currencyGreaterThan(currencyGreater).currencyLessThan(currencyLesser).limit(limit)
      .nextToken(nextToken)
      .do();
  });

  When('I get the next page using {int} to lookup asset balances for {int} with {int}, {int}, {int}', async (clientNum, assetIndex, currencyGreater, currencyLesser, limit) => {
    const ic = indexerIntegrationClients[clientNum];
  ...
  Then('There are {int} with the asset, the first is {string} has {string} and {int}', (numAccounts, firstAccountAddress, isFrozenString, accountAmount) => {
  ...
  When('I use {int} to search for an account with {int}, {int}, {int}, {int} and token {string}', async (clientNum, assetIndex, limit, currencyGreater, currencyLesser, nextToken) => {
    const ic = indexerIntegrationClients[clientNum];
    integrationSearchAccountsResponse = await ic.searchAccounts().assetID(assetIndex).currencyGreaterThan(currencyGreater).currencyLessThan(currencyLesser)
      .limit(limit)
      .nextToken(nextToken)
      .do();
  });
  ...
  Then('There are {int}, the first has {int}, {int}, {int}, {int}, {string}, {int}, {string}, {string}', (numAccounts, pendingRewards, rewardsBase, rewards, withoutRewards, address, amount, status, type) => {
  ...
  Then('I get the next page using {int} to search for an account with {int}, {int}, {int} and {int}', async (clientNum, assetIndex, limit, currencyGreater, currencyLesser) => {
    const ic = indexerIntegrationClients[clientNum];
  ...
  Then('The first account is online and has {string}, {int}, {int}, {int}, {string}, {string}', (address, keyDilution, firstValid, lastValid, voteKey, selKey) => {
    const scrutinizedAccount = integrationSearchAccountsResponse.accounts[0];
  ...
  When('I use {int} to search for transactions with {int}, {string}, {string}, {string}, {string}, {int}, {int}, {int}, {int}, {string}, {string}, {int}, {int}, {string}, {string}, {string} and token {string}', async (clientNum, limit, notePrefix, txType, sigType, txid, round, minRound, maxRound, assetId, beforeTime, afterTime, currencyGreater, currencyLesser, address, addressRole, excludeCloseToString, nextToken) => {
    const ic = indexerIntegrationClients[clientNum];
    const excludeCloseToBool = (excludeCloseToString == 'true');
    integrationSearchTransactionsResponse = await ic.searchForTransactions().limit(limit).notePrefix(notePrefix).txType(txType)
      .sigType(sigType)
      .txid(txid)
      .round(round)
      .minRound(minRound)
  ...
  When('I use {int} to search for all {string} transactions', async (clientNum, account) => {
    const ic = indexerIntegrationClients[clientNum];
    integrationSearchTransactionsResponse = await ic.searchForTransactions().address(account).do();
  });

  When('I use {int} to search for all {int} asset transactions', async (clientNum, assetIndex) => {
    const ic = indexerIntegrationClients[clientNum];
  ...
  When('I use {int} to search for applications with {int}, {int}, and token {string}', async function (clientNum, limit, appID, token) {
    const ic = indexerIntegrationClients[clientNum];
    this.responseForDirectJsonComparison = await ic.searchForApplications().limit(limit).index(appID).nextToken(token)
      .do();
  });
  ...
  When('I get the next page using {int} to search for transactions with {int} and {int}', async (clientNum, limit, maxRound) => {
    const ic = indexerIntegrationClients[clientNum];
    const nextToken = integrationSearchTransactionsResponse['next-token'];
    integrationSearchTransactionsResponse = await ic.searchForTransactions().limit(limit).maxRound(maxRound).nextToken(nextToken)
      .do();
  });

  Then('there are {int} transactions in the response, the first is {string}.', (numTransactions, txid) => {
  ...
  Then('Every transaction has tx-type {string}', (txType) => {
    for (idx = 0; idx < integrationSearchTransactionsResponse.transactions.length; idx++) {
      const scrutinizedTxn = integrationSearchTransactionsResponse.transactions[idx];
  ...
  Then('Every transaction moves between {int} and {int} currency', (lowerBound, upperBound) => {
  ...
  When('I use {int} to search for assets with {int}, {int}, {string}, {string}, {string}, and token {string}', async (clientNum, zero, assetId, creator, name, unit, nextToken) => {
    const ic = indexerIntegrationClients[clientNum];
    integrationSearchAssetsResponse = await ic.searchForAssets().index(assetId).creator(creator).name(name)
      .unit(unit)
      .nextToken(nextToken)
      .do();
  });

  Then('there are {int} assets in the response, the first is {int}.', (numAssets, firstAssetId) => {
  ...
  Then('the parsed Dryrun Response should have global delta {string} with {int}', (key, action) => {
  ...
  Then('it is compiled with {int} and {string} and {string}', (status, result, hash) => {
  ...
  Given('I create a new transient account and fund it with {int} microalgos.', async function (fundingAmount) {
    ...
    const stxKmd = await this.kcl.signTransaction(this.handle, this.wallet_pswd, fundingTxnArgs);
    ...
    const fundingConfirmation = await this.acl.transactionById(fundingResponse.txId);
  ...
  Given('I remember the new application ID.', async function () {
    const infoResult = await this.acl.pendingTransactionInformation(this.appTxid.txId);
  ...
  Then('The transient account should have the created app {string} and total schema byte-slices {int} and uints {int},'
    ...
    stateValue) {
    const accountInfo = await this.v2Client.accountInformation(this.transientAddress).do();
