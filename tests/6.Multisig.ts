...
describe('Multisig Functionality', () => {
  describe('should generate correct partial signature', () => {
    it('first partial sig should match golden main repo result', () => {
      const txn = algosdk.makePaymentTxnWithSuggestedParamsFromObject({
        ...
        amount: 1000,
        note: new Uint8Array(Buffer.from('RSYiABhShvs=', 'base64')),
        closeRemainderTo:
      ...
      const expectedSignedTxn = Buffer.from(
      ...
      assert.deepStrictEqual(Buffer.from(blob), expectedSignedTxn);
    });
    ...
    it('second partial sig should match golden main repo result', () => {
      ...
      assert.deepStrictEqual(Buffer.from(mergedBlob), expectedMergedBlob);
    });
  });
  ...
  describe('should sign keyreg transaction types', () => {
    it('first partial sig should match golden main repo result', () => {
      ...
      const oneSigTxBlob = Buffer.from(
      ...
      assert.deepStrictEqual(Buffer.from(blob), oneSigTxBlob);
    });

    it('second partial sig with 3rd pk should match golden main repo result', () => {
      const rawOneSigTxBlob = Buffer.from(
        '',
        'base64'
      );
      ...
      const twoSigTxBlob = Buffer.from(
