...
function isValidSeed(seed) {
  ...
  let checksum = nacl.genericHash(decoded.seed).slice(nacl.SEED_BYTES_LENGTH - ALGORAND_CHECKSUM_BYTE_LENGTH, nacl.SEED_BYTES_LENGTH); // Check if the checksum and the seed are equal
...
function decode(seed) {
  if (!(typeof seed === "string" || seed instanceof String)) throw MALFORMED_SEED_ERROR; //try to decode

  let decoded = base32.decode.asBytes(seed); // Sanity check
...
function encode(secretKey) {
  // get seed
  const seed = secretKey.slice(0, nacl.SEED_BYTES_LENGTH); //compute checksum
