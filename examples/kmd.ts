...
async function main() {
  // example: JSSDK_KMD_CREATE_CLIENT
  ...
  // example: JSSDK_KMD_CREATE_WALLET
  ...
  const wallet = (await kmdclient.createWallet(walletName, password, masterDerivationKey, driver));
  const walletID = wallet.wallet.id;
  console.log('Created wallet:', walletID);
  // example: JSSDK_KMD_CREATE_WALLET

  // example: JSSDK_KMD_CREATE_ACCOUNT
  ...
  // example: JSSDK_KMD_RECOVER_WALLET
  const exportedMDK = (await kmdclient.exportMasterDerivationKey(wallethandle, 'testpassword')).master_derivation_key;
  const recoveredWallet = (await kmdclient.createWallet('testWallet2', 'testpassword', exportedMDK, 'sqlite'));
  const recoeveredWalletID = recoveredWallet.wallet.id;

  console.log('Created wallet: ', recoeveredWalletID);

  const recoveredWalletHandle = (await kmdclient.initWalletHandle(recoeveredWalletID, 'testpassword')).wallet_handle_token;
  console.log('Got wallet handle: ', recoveredWalletHandle);

  const recoveredAddr = (await kmdclient.generateKey(recoveredWalletHandle)).address;
